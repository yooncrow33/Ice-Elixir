...
class ExitPopup() {
    ...
    fun render(g: Graphics, message: String, title: String) {
        ...
        // 3. 텍스트 렌더링
        g.setColor(Color(240,240,240)) // 타이틀: 밝은 아이스 블루
        g.setFont(Font("Impact", Font.BOLD, 32))
        g.drawString(title, px + 30, py + 50)

        g.setColor(Color(210,210,210)) // 본문: 아주 연한 푸른 흰색
        ...
        // [ABORT] 버튼 (Yes 역할)
        if (isYesSelected) {
            g.setColor(Color(255,50,30)) // 활성화 시 진한 하늘색
        ...
        // [CONTINUE] 버튼 (No 역할)
        if (!isYesSelected) {
            g.setColor(Color(180, 180, 180)) // 활성화 시 밝은 아이스 블루
